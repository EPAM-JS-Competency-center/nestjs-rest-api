...
# For Ubuntu server with NVM:
#ssh -i "${PATH_TO_CERTIFICATE}" "${EC2_URI}" ". ~/.nvm/nvm.sh; pm2 delete all || echo 'nothing to delete' && export PORT=${PORT}; export NODE_ENV=${NODE_ENV}; pm2 start /var/www/${APP_DIR}/main.js -f"
